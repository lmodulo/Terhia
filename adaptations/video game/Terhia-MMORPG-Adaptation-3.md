...
>
> **Canon read for this pass.** v0.1.1: Parts I–XI except XI.2, plus R.5 and R.8; not read: R.0–R.4 and Appendix H. v0.2-draft: none.
>
> **Session load for follow-up work.** This file, about 21k tokens, plus Parts II–IV (schools, ranks, costs) and Part X (visual grammar), about 8.8k: about 30k tokens, against about 61k for the full canon.
...
|:--|:--|:--|
| Cost | Canon | What practice does to a practitioner's body and life, per school (Parts II–IV). Casting cost varies with moon phase only (ratified 11 September 2026) [C: section citation owed] |
...
| Necromancer | Ranger (names the killer of a stealth kill); Apostle (execute marks end targets its heals keep near defeat) | Berserker |
...
Phase-turn days are market truces in Renlei homelands, where canon records the custom [C: Renlei custom; section citation owed]. Other homelands trade normally at the turn [A].
...
- **Guru** places Dilation Volumes to slow enemies or speed up allied recovery. The player sets radius, ratio, and duration, and each multiplies cost [C: Part II, Arcane Guru; section citation owed]. Three tiers each, so 1× to 27× base (placeholder) [A]. Gravity Vectors pin a target, arrest a fall, or redirect a leap or projectile. Edits need the Overlay running and hold only while channeled; a stagger ends them. A volume that contains the caster multiplies strain and builds desync, per exterior-placement doctrine.
...
That makes the healer the group's scarcest resource. At formation level, canon's Crown column is Hammer (Paladin) with Archpriest and Apostle support [N: Force Types]; the group template follows that column [A].
...
|:--|:--|:--|:--|
| Sorcerer | Master | Time; gravity | Time: the Overlay's lookahead extends (placeholder ×1.5) and shows enemy cast and recovery timings. Gravity: Novice telekinesis scales with the Adept read, throwing heavier objects and shoving farther. No edit of time or gravity below Guru |
...
| Archpriest | Adept, discovered in play; formalized at Master (Part II, Elemental Master) [C] | Fire; water; wind; stone | Not Guru-only |
...
**Navigation.** Tangata channels require the channel's tale, whose text encodes the hazards. Accord charts miss those hazards ("a better wreck record"). Rotokere has no tale and is navigated without one, unexplained. The ninth channel off Puketai opens only in Velquor and stays unclaimed [R, section 9: Ninth channel as a route].
...
**Presentation.** HUD margins, camera distance, and audio compression track the encounter stage: stable, rising, crisis. Removing the HUD entirely is reserved for Guru-tier moments and capped per season. That is the game's version of Part X's limit of two full-bleed pages per volume [C: Part X].
...
| Strain modifiers beyond phase | Casting cost varies with moon phase only (ratified 11 September 2026). The Arcane Guru dilation parameters multiply cost (Part II) | Default: kept as strain modifiers, which the section 3 glossary defines as game quantities, not canon cost. Affected: Sorcerer read-target discount and rush surcharge; Archpriest present-material discount and concurrent-works multiplier. If rejected, the Sorcerer's rhythm skill moves to cast-timing windows, and Archpriest upkeep keeps a flat per-work rate |
...
| Nature wound redistribution | Master channel redirection through sustained contact is canon; moving a wound between people is not stated | Default: the Master kit moves a wound between linked allies and spreads damage across a linked group (section 4). Alternative: channel work acts only on the contact target |
| Portal access by registration | Portals run under manifest; no line recorded in the handoff ties passage to practitioner registration | Default: registered characters get portal access (section 6). Alternative: portals take anyone who pays the fare, and registration changes only manifest inspection |
| Witness law for kills | The Menneske three-witness rule does not extend to kills | Default: a kill before three witnesses becomes a bounty record (section 6). Alternative: witness law covers what canon covers, and bounties come from charter enforcement |

### Canon dependencies

Design elements that rest on pending canon [N]. Locations are as recorded in the handoff; an entry reading to locate means the R.1 item is not yet identified. Status is unconfirmed: the canon pass checks each row against R.0 and R.1. If a row is rejected, the listed elements need rework.

| Pending canon | Where recorded | Design elements that depend on it |
|:--|:--|:--|
| Platoon standard, forty foot | Block 3, row 3j | Power curves (section 2); formation stats (section 3) |
| Phase Cascade | D2, Block 2 | Martial and alchemy classes phase-neutral (section 3) |
| Biome × Moon grid extension, six biomes | R.1, to locate | Phase-week hazards (section 3); seasonal reading (section 3) |
| Appendix D, prior Guru-tier events | R.1, to locate | Velquor Guru-tier window (section 3) |
| Material Signatures fiber, craft, and palette cells | Block 3, row 3g | Culture art specs (section 8) |
| Economic Baseline; Money; Anchor Prices; Population Magnitudes | R.1 item 26 (Block 2); Block 3 rows 3b | Economy, including NPC baseline prices (section 6) |
| Substrate and Nodes; the Three Readings | R.1, to locate | Node definitions (section 5); territory claims (section 6) |
| Practitioner, latent and trained | D1, Block 2 | Trade knacks (section 6) |
| Transport table; airship propulsion | R.1, to locate | Transport rules (section 6) |
| Force Types | [V/N], to locate | Formation system (section 3); Apostle group template (section 4) |
| Rank Distribution (Ledger statistic): the rolls of 23 and 14, the unregistered count, cost first on the file at Adept, a rolled Guru's file as a matter of treaty | Block 3, row 3a (R.1 item 29) | Cost-file onset and Guru files (section 3); roll sizing, boards reading the file, unregistered cap (section 7); Guru population row (this section) |
...
Rows added in v0.2-draft without a numbered decision: Nature wound redistribution, Portal access by registration, Witness law for kills. Each keeps its default until ruled.
