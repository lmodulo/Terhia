...
Each entry gives: the fantasy; key attribute, armor, Vigor type; the **signature resource** (the little game only this class plays); the **Core** (the 0-Strain ability that changes form at every rank); the **Reading** and its exploit; the ability list by rank (**Novice A and B at level 1, C at 3; Adept A at 5, B at 7; Master A at 9, B at 11; Guru A at 13, B at 15** — §13.1); and the four canon cost stages. Damage entries add the Key modifier once per hit unless noted.
...
- **Novice B · Adhesive** · Control · Quick · 1 · one target within 8 vs Guard · Rooted 1 round (Large: Slowed instead). A Quick, so it stacks with the *Flask*: root it, then hit it.
...
Every rule that touches a canon sentence, with the reading taken. Nothing here is canon until Joe rules; Canon itself is untouched. Rows marked *ruled* carry the ruling's number from Terhia-Heroes-Rulings.md (v0.3); rows H42–H50 are new in v0.3; **rows H51–H59 are new in v0.4** and H60 in v0.5; H51–H59 point at the Cultures-and-Factions module's CF-rows, which hold the full readings. Numbering is Heroes-local (H-rows) so the register never collides with Chronicle's.
